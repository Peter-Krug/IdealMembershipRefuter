...
def run_cadical_with_cnf(cnf: CNF, cadical_path="/mnt/c/Users/p98kr/sage_local_work/cadical/build/cadical"):
    """
...
def construct_counterexample(assumptions,claims,Q=None,dimensions=None,n=2,random_tries=0,verbosity=0,one_lifting=True,fix_a=False):
    r"""
    Input:
...
def lift_solution(pol_arr,n,mat_dic,f=None,deterministic=True):
    ...
    possible_solutions = [tuple(vec) for vec in basis]
    if not is_1:
        zero_vec = tuple([0]*C.ncols())  # tuple zero vector
        possible_solutions = [zero_vec] + possible_solutions  # all tuples now
    if is_1:
    ...
    if not deterministic and len(possible_solutions) > 1:
        coeffs = [ZZ.random_element(0, 2) for _ in possible_solutions]

        zero_tuple = tuple([0]*len(possible_solutions[0]))
        
        
        random_combo = sum((coeff * vec for coeff, vec in zip(coeffs, possible_solutions)), zero_tuple)
        possible_solutions = [random_combo] + possible_solutions[1:]
...
def get_zeros(assumptions,fixed_matrix_name = "a", max_zeros=10, min_dim=2, max_dim=5, fix_type=0,lift_exp=2,deterministic=False,lift_tries=4):
    ...
    while dim <= max_dim:
        ...
        while True:
            ...
            for _ in range(lift_tries):
                ...
                check_counterexample(assumptions, one_sol_dict, Q=None)
                try:
                    one_sol_dict = multi_lift_solution(assumptions, lift_exp, mats, deterministic=False)
                    if check_counterexample(assumptions, one_sol_dict, Q=None):
                        all_matrices.append(one_sol_dict)
            ...
            if len(all_matrices) >= max_zeros + 1:
                del all_matrices[0]
    ...
    # Pretty-print solutions before returning
    print(f"\n=== Found {len(all_matrices)-1} zero solutions ===")
    for idx, sol in enumerate(all_matrices):
        print(f"\n--- Solution #{idx+1} ---")
        for key, matrix in sol.items():
            if key.endswith("_adj"):
